...
struct Config
{
	...
	void loadCLI(string[] cliArgs)
	{
		...
		try
		{
			// -h/--help will not pass through due to the autogenerated help option
			auto firstResult = getopt(cliArgs, std.getopt.config.caseSensitive,
			       std.getopt.config.passThrough, "config|F", &configFile);
			...
			auto getoptResult = getopt(cliArgs, std.getopt.config.caseSensitive,
			       "css|c",                    &cssFileName,
			       "generate-css|C",           &doGenerateCSSPath,
			       "exclude|e",                &newExcludes,
			       "format|f",                 &format,
			       "generate-cfg|g",           &doGenerateConfig,
			       ...
			       "toc-additional|t",         &tocAdditionalFileNames,
			       "toc-additional-direct|T",  &tocAdditionalStrings,
			       "max-module-list-length|l", &maxModuleListLength
			       );
		}
		catch(Exception e)
		{
